			...
			<List class="space-y-4">
				{#if pullRequests.length > 0}
					{#each pullRequests as pr}
						<Item
							class="bg-white p-4 rounded-lg shadow-lg hover:bg-blue-100 cursor-pointer"
							tabindex="0"
	...
	{:else if !selectedReview}
		<div>
			<div class="flex items-center">
				...
				<h2 class="text-2xl font-bold mb-6 text-primary">
					Reviews for {selectedPullRequest.title}
				</h2>
			</div>
			<List class="space-y-4">
				{#if reviews.length > 0}
					{#each reviews as review}
						<Item
							class="bg-white p-4 rounded-lg shadow-lg cursor-pointer hover:bg-blue-100"
							tabindex="0"
				...
				{#if isAddingReview}
					...
					>
						<label for="reviewName" class="block text-sm font-medium text-gray-700"
							>Review Name</label
						>
						<input
							id="reviewName"
						...
						<div class="mt-4 flex justify-end space-x-2">
							...
							<Button
								...
								on:click={() => (isAddingReview = false)}>Cancel</Button
							>
						</div>
					</div>
				{/if}
