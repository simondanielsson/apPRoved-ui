...
{#if node.type === 'folder'}
	<div>
		<!-- Folder Item -->
		<Item
			class="cursor-pointer hover:bg-gray-200 rounded-lg flex items-center mb-2"
		...
		>
			<span>
				<svg
					...
					viewBox="0 0 24 24"
					stroke="currentColor"
					class="w-6 h-6 transition-transform transform duration-200 {isCollapsed
						? ''
						: 'rotate-90'}"
				>
		...
		<div class="ml-3 folder-children" class:hidden={isCollapsed}>
			{#each Object.keys(node.children) as key}
				<svelte:self
					...
					{selectedFileReview}
					on:selectFileReview={handleFileReviewSelected}
				/>
			{/each}
		</div>
	</div>
{:else}
	<!-- File -->
	<Item
		class="pl-2 ml-3 mb-2 cursor-pointer flex items-center hover:bg-gray-200 rounded-lg {isSelected
			? 'bg-gray-200'
			: ''}"
		on:click={() => handleSelectFileReview(node.fileReview)}
	>
