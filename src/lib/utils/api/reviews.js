...
export async function fetchRepositories(cookies) {
  const response = await fetch(`${API_URL}/repositories`, { headers: { 'Authorization': `Bearer ${cookies.get('token')}` } });
...
export async function fetchReviews(repositoryId, pullRequestId, fetch) {
  return await fetch(`${API_URL}/repositories/${repositoryId}/pull-requests/${pullRequestId}/reviews`);
}

export async function fetchFileReviews(repositoryId, pullRequestId, reviewId, fetch) {
  return await fetch(`${API_URL}/repositories/${repositoryId}/pull-requests/${pullRequestId}/reviews/${reviewId}`);
}
...
export async function getReviewStatus(repoID, prID, reviewID, fetch) {
  return await fetch(`${API_URL}/repositories/${repoID}/pull-requests/${prID}/reviews/${reviewID}/progress`);
}

export async function deleteReview(repoID, prID, reviewID, fetch) {
  return await fetch(`${API_URL}/repositories/${repoID}/pull-requests/${prID}/reviews/${reviewID}`, {
    method: 'DELETE',
  });
}
