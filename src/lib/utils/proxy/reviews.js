...
export async function getReviewStatus(repoID, prID, reviewID, fetch) {
  const response = await fetch(`/app/repositories/${repoID}/pull-requests/${prID}/reviews/${reviewID}/status`)
  if (!response.ok) {
...
export async function fetchFileReviews(repositoryId, pullRequestId, reviewId, fetch) {
  const response = await fetch(`/app/repositories/${repositoryId}/pull-requests/${pullRequestId}/reviews/${reviewId}`);
  if (response.status != 200) {
...
export async function deleteReview(repoID, prID, reviewID, fetch) {
  const response = await fetch(`/app/repositories/${repoID}/pull-requests/${prID}/reviews/${reviewID}`, {
    method: 'DELETE',
  });
  if (response.status != 200) {
    return {
