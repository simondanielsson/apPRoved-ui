...
<div class="p-4 overflow-y-auto max-h-[calc(100vh-120px)] flex flex-col">
	...
	<List class="space-y-4 overflow-y-auto">
			...
			{#each pullRequests as pr}
				...
				>
					<Item class="bg-white p-4 rounded-lg shadow-lg hover:bg-blue-100 cursor-pointer">
						<h3 class="{pr.state === 'open' ? 'text-lg' : 'text-gray-700'} font-semibold">
		...
		<div class="mt-8 p-4 flex justify-center sticky bottom-0 bg-gray-50 rounded-lg shadow-sm">
			<Button
				class="flex items-center text-primary hover:text-primary-dark {isRefreshing
			...
			>
				<svg
					...
					stroke-width="1.5"
					stroke="currentColor"
					class="size-6 {isRefreshing ? 'animate-spin' : ''}"
				>
					<path
				...
				<span class="flex items-center ml-2">{isRefreshing ? 'Refreshing...' : 'Refresh'}</span>
			</Button>
		</div>
	</List>
</div>
