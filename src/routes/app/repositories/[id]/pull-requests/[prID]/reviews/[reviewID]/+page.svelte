...
<div class="w-full h-full flex">
	<div class="flex flex-col window-h w-full">
		...
		<div class="flex flex-row items-center mb-4">
			...
			<h1 class="text-3xl w-full ml-4 font-bold text-primary">
				Review for {$selectedPullRequest ? $selectedPullRequest.title : 'loading...'}
			</h1>
		</div>
				...
				<List class="space-y-2">
					{#each Object.keys(fileTree) as key}
						<FileTreeNode
							node={fileTree[key]}
							{selectedFileReview}
							on:selectFileReview={handleSelectFileReview}
						/>
					{/each}
				</List>
			</div>
