<script>
	...
	function updatePendingReviewProgress(reviewId, progress) {
		reviews = reviews.map((review) => (review.id === reviewId ? { ...review, progress } : review));
	}
...
<div>
	<div class="flex items-center">
		...
		<h2 class="text-2xl font-bold mb-6 text-primary">
			Reviews for {$selectedPullRequest
				? `${$selectedPullRequest.title} #${$selectedPullRequest.number}`
				: 'Loading...'}
		</h2>
	</div>
	...
	<List class="space-y-4">
			...
			{#each reviews as review}
				...
				>
					<div class="flex flex-row items-center">
						...
						>
							<h3 class="text-lg {isPending(review) ? 'text-gray-500' : ''} font-semibold">
								{review.title}
							</h3>
							<span class={isPending(review) ? 'text-gray-500' : 'text-gray-700'}
								>Created at <Time
					...
					{#if isPending(review)}
						<div class="w-full mt-2">
							<ProgressBar
								series={progressToSeries(review.progress)}
								showProgressValue={false}
								ry="1.5"
								rx="1.5"
								cls="rounded rounded-lg"
							/>
						</div>
					{/if}
				</div>
		...
		<Item
			class="mt-6 w-full bg-primary text-white p-4 rounded-lg shadow-lg hover:bg-primary-dark flex items-center justify-center space-x-2 cursor-pointer"
			tabindex="0"
		...
		{#if isAddingReview}
			<div class="mt-4 p-4 bg-gray-100 rounded-lg shadow-sm" transition:fade={{ duration: 150 }}>
				...
				<div class="mt-4 flex justify-end space-x-2">
					<Button
						class="bg-primary text-white p-2 rounded-lg shadow-lg hover:bg-primary-dark"
						on:click={handleCreateReview}>Submit</Button
					>
					<Button
						class="bg-gray-300 text-black p-2 rounded-lg shadow-md hover:bg-gray-400"
						on:click={resetCreateReview}>Cancel</Button
					>
				</div>
			</div>
		{/if}
...
<Modal isOpen={isModalOpen} on:confirm={confirmDeleteReview} on:cancel={cancelDeleteReview}>
